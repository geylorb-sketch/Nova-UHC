...
import net.novaproject.novauhc.lang.scenario.SimonSaysLang;
...
public class SimonSays extends Scenario {
    ...
    private String t(SimonSaysLang key) { return LangManager.get().get(key); }
        ...
        simonTask = (BukkitRunnable) new BukkitRunnable() {
            ...
            @Override public void run() {
                ...
                if (!commandActive) {
                    ...
                    else if (nextCommandIn - timer == 30) Bukkit.broadcastMessage(t(SimonSaysLang.INCOMING_ORDER));
                } else {
                    commandTimeLeft--;
                    if (commandTimeLeft <= 0) endCommand();
                    else if (commandTimeLeft == 5) Bukkit.broadcastMessage(t(SimonSaysLang.FIVE_SECONDS));
                }
            }
    ...
    private void issueRandomCommand() {
        ...
        Bukkit.broadcastMessage(t(SimonSaysLang.NEW_ORDER, Map.of("%order%", currentCommand.description)));
        Bukkit.broadcastMessage(t(SimonSaysLang.ORDER_DURATION, Map.of("%seconds%", currentCommand.duration)));
        for (UHCPlayer p : UHCPlayerManager.get().getPlayingOnlineUHCPlayers())
            p.getPlayer().playSound(p.getPlayer().getLocation(), org.bukkit.Sound.NOTE_PLING, 1f, 1.5f);
    ...
    private void endCommand() {
        ...
        if (!failed.isEmpty()) {
            ...
            Bukkit.broadcastMessage(t(SimonSaysLang.FAILED_COUNT, Map.of("%count%", failed.size())));
        } else {
            Bukkit.broadcastMessage(t(SimonSaysLang.ALL_OBEYED));
        }
    ...
    private void applyPenalty(Player player) {
        ...
        switch (penalties.get(random.nextInt(penalties.size()))) {
            case DAMAGE -> { player.damage(2.0); player.sendMessage(t(SimonSaysLang.DAMAGE_MSG)); }
            case HUNGER -> { player.setFoodLevel(Math.max(0, player.getFoodLevel() - 4)); player.sendMessage(t(SimonSaysLang.HUNGER_MSG)); }
            case SLOWNESS -> player.addPotionEffect(new org.bukkit.potion.PotionEffect(org.bukkit.potion.PotionEffectType.SLOW, 200, 1));
            case BLINDNESS -> player.addPotionEffect(new org.bukkit.potion.PotionEffect(org.bukkit.potion.PotionEffectType.BLINDNESS, 100, 0));
            case WEAKNESS -> player.addPotionEffect(new org.bukkit.potion.PotionEffect(org.bukkit.potion.PotionEffectType.WEAKNESS, 200, 0));
        }
    }
