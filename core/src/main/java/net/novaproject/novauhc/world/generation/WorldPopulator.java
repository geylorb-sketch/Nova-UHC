...
public class WorldPopulator {
    ...
    public WorldPopulator(World arena, CenterType type, Biome biome) {
        this.arena = arena;
        this.type = type;
        if(!LoadingChunkTask.get().isFinished()){
    ...
    public void generateForest(TreeType one, TreeType two, TreeType three,
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                String progressBar = ProgressBar.getProgressBar(
                        progress,
                        maxProgress,
                        40,
                        "|",
                ...
                for (Player player : Bukkit.getOnlinePlayers()) {
                    new Titles().sendActionText(
                            player,
                            LangManager.get().get(
                                    TaskLang.FOREST_GENERATION_ACTION_BAR,
                                    player,
                                    java.util.Map.of(
                                            "%percent%", formatted,
                                            "%bar%", progressBar
    ...
    private void sendProgress(String message, int progress, int max) {
        String bar = ProgressBar.getProgressBar(
                progress,
                max,
                20,
                "|",
