...
/**
 * Espace Vide — Kinta
 * TP joueurs 30 blocs dans dimension. Kinta obtient son armure gold renforcée.
 */
public class EspaceVideKintaAbility extends UseAbiliy {

    private final Random random = new Random();

    @Override public String getName() { return "Espace Vide"; }
    @Override public Material getMaterial() { return Material.ENDER_PEARL; }

    public EspaceVideKintaAbility() { setCooldown(600); }
    ...
    public boolean onEnable(Player player) {
        ...
        for (UHCPlayer uhc : UHCPlayerManager.get().getPlayingOnlineUHCPlayers()) {
            ...
            teleported.add(p);
            if (!p.equals(player)) p.sendMessage("§5✦ Aspiré dans l'espace vide de Kinta !");
        }

        player.sendMessage("§e§l✦ Espace Vide activé !");
        setCooldown(600);
