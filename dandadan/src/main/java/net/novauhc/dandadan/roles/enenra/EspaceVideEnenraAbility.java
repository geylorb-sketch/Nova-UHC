...
public class EspaceVideEnenraAbility extends UseAbiliy {
    ...
    public boolean onEnable(Player player) {
        ...
        for (UHCPlayer uhc : UHCPlayerManager.get().getPlayingOnlineUHCPlayers()) {
            ...
            teleported.add(p);
            if (!p.equals(player)) p.sendMessage("§5✦ Aspiré dans l'espace vide — Brouillard !");
        }

        player.sendMessage("§5§l✦ Brouillard activé !");
        setCooldown(600);
