...
/**
 * Dimension — Espace Vide de Rokuro
 * TP joueurs dans un rayon de 30 blocs dans dimension pendant 1min.
 */
public class EspaceVideRokuroAbility extends UseAbiliy {
    ...
    public EspaceVideRokuroAbility() { setCooldown(600); }
    ...
    public boolean onEnable(Player player) {
        ...
        for (UHCPlayer uhc : UHCPlayerManager.get().getPlayingOnlineUHCPlayers()) {
            ...
            teleported.add(p);
            if (!p.equals(player)) p.sendMessage("§5✦ Aspiré dans l'espace vide — Dimension !");
        }

        player.sendMessage("§5§l✦ Dimension activé !");
        setCooldown(600);
