...
/**
 * Toile — Espace Vide de Acrobatique
 * TP joueurs dans un rayon de 30 blocs dans dimension pendant 1min.
 */
public class EspaceVideAcrobatiqueAbility extends UseAbiliy {

    private final Random random = new Random();
    private final List<Player> teleported = new ArrayList<>();

    @Override public String getName() { return "Toile"; }
    @Override public Material getMaterial() { return Material.ENDER_PEARL; }

    public EspaceVideAcrobatiqueAbility() { setCooldown(600); }
    ...
    public boolean onEnable(Player player) {
        ...
        for (UHCPlayer uhc : UHCPlayerManager.get().getPlayingOnlineUHCPlayers()) {
            ...
            teleported.add(p);
            if (!p.equals(player)) p.sendMessage("§5✦ Aspiré dans l'espace vide — Toile !");
        }

        player.sendMessage("§5§l✦ Toile activé !");
        setCooldown(600);
