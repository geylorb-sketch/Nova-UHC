...
public class EspaceVideFlatwoodsAbility extends UseAbiliy {
    ...
    public boolean onEnable(Player player) {
        ...
        for (UHCPlayer uhc : UHCPlayerManager.get().getPlayingOnlineUHCPlayers()) {
            ...
            teleported.add(p);
            if (!p.equals(player)) p.sendMessage("§5✦ Aspiré dans l'espace vide — Forêt !");
        }

        player.sendMessage("§5§l✦ Forêt activé !");
        setCooldown(600);
