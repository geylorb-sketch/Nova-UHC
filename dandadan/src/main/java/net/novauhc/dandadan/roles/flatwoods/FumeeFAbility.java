package net.novauhc.dandadan.roles.flatwoods;

import net.novaproject.novauhc.ability.template.UseAbility;
import net.novaproject.novauhc.lang.LangManager;
import net.novauhc.dandadan.lang.DanDaDanLang;
import org.bukkit.Material;
import org.bukkit.entity.Entity;
import org.bukkit.entity.Player;
import org.bukkit.potion.PotionEffect;
import org.bukkit.potion.PotionEffectType;

import java.util.Map;

public class FumeeFAbility extends UseAbility {
    @Override public String getName() { return "Fumee"; }
    @Override public Material getMaterial() { return Material.COAL; }
    @Override public boolean onEnable(Player p) {
        for (Entity e : p.getNearbyEntities(10,10,10)) {
            if (e instanceof Player t) t.addPotionEffect(new PotionEffect(PotionEffectType.BLINDNESS, 100, 0, false, true));
        }
        LangManager.get().send(DanDaDanLang.GENERIC_ABILITY_ON, p, Map.of("%name%", getName()));
        setCooldown(300); return true; }
}
