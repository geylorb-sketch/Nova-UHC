...
public class EspaceVideJetBoosterAbility extends UseAbiliy {
    ...
    public boolean onEnable(Player player) {
        ...
        for (UHCPlayer uhc : UHCPlayerManager.get().getPlayingOnlineUHCPlayers()) {
            ...
            teleported.add(p);
            if (!p.equals(player)) p.sendMessage("§5✦ Aspiré dans l'espace vide — Ciel !");
        }

        player.sendMessage("§5§l✦ Ciel activé !");
        setCooldown(600);
