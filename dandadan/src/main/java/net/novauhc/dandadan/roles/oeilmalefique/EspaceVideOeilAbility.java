...
/**
 * Iris — Espace Vide de Œil Maléfique
 * TP joueurs dans un rayon de 30 blocs dans dimension pendant 1min.
 */
public class EspaceVideOeilAbility extends UseAbiliy {
    ...
    public EspaceVideOeilAbility() { setCooldown(600); }
    ...
    public boolean onEnable(Player player) {
        ...
        for (UHCPlayer uhc : UHCPlayerManager.get().getPlayingOnlineUHCPlayers()) {
            ...
            teleported.add(p);
            if (!p.equals(player)) p.sendMessage("§5✦ Aspiré dans l'espace vide — Iris !");
        }

        player.sendMessage("§5§l✦ Iris activé !");
        setCooldown(600);
