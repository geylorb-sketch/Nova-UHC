...
/**
 * Miroir — Espace Vide de Reiko
 * TP joueurs dans un rayon de 30 blocs dans dimension pendant 1min.
 */
public class EspaceVideReikoAbility extends UseAbiliy {
    ...
    public EspaceVideReikoAbility() { setCooldown(600); }
    ...
    public boolean onEnable(Player player) {
        ...
        for (UHCPlayer uhc : UHCPlayerManager.get().getPlayingOnlineUHCPlayers()) {
            ...
            teleported.add(p);
            if (!p.equals(player)) p.sendMessage("§5✦ Aspiré dans l'espace vide — Miroir !");
        }

        player.sendMessage("§5§l✦ Miroir activé !");
        setCooldown(600);
